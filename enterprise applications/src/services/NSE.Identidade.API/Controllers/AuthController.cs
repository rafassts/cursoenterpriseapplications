...
namespace NSE.Identidade.API.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        {
            //nesse ponto não temos o id, então precisamos recuperar do banco
            var usuario = await _authenticationService.UserManager.FindByEmailAsync(usuarioRegistro.Email);

            var usuarioRegistrado = new UsuarioRegistradoIntegrationEvent(
                Guid.Parse(usuario.Id), usuarioRegistro.Nome, usuarioRegistro.Email, usuarioRegistro.Cpf);

            //se estourar exception, não vai retornar para excluir após o retorno desta função, então temos que tratar aqui
            try
            {
