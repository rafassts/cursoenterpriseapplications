...
namespace NSE.Identidade.API.Services
{
    ...
    {
        //dependências do identity
        public readonly SignInManager<IdentityUser> SignInManager; //logar
        public readonly UserManager<IdentityUser> UserManager; //criar
        ...
        //jwks - não tem mais audiência
        private string CodificarToken(ClaimsIdentity identityClaims)
        {
            var tokenHandler = new JwtSecurityTokenHandler();
            var currentIssuer = //próprio endopoint da api
                $"{_aspNetUser.ObterHttpContext().Request.Scheme}://{_aspNetUser.ObterHttpContext().Request.Host}";
            var key = _jwksService.GetCurrent(); //jwk - gera a chave aleatoriamente
            var token = tokenHandler.CreateToken(new SecurityTokenDescriptor
            {
                Issuer = currentIssuer, //muda por causa do jwks
                Subject = identityClaims,
                Expires = DateTime.UtcNow.AddHours(1), //padrão 1 hr
