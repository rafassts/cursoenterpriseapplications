...
{
    //fila rpc - remote procedure call, vai esperar o retorno (request-response) para o evento de pedido iniciado
    //fila pubsub - publish subscribe, recebe mensagens de pedidobaixadoestoque ou pedidocancelado
    //autorizar pagamento - reserva o crédito / capturar pagamento - efetivar
    public class PagamentoIntegrationHandler : BackgroundService
    {
        ...
        {
            //quando iniciar um pagamento, vai ser enviado o request da api de pedido e aqui responde
        ...
        private async Task CancelarPagamento(PedidoCanceladoIntegrationEvent message)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                //a mensagem PedidoCanceladoIntegrationEvent está sendo ouvido no pedido também, então não precisa retornar nada
            }
        }
        ...
        private async Task CapturarPagamento(PedidoBaixadoEstoqueIntegrationEvent message)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                //se deu certo, publica a mensagem que o pedido foi pago, api de pedidos conclui
